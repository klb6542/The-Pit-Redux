...
public class KingBuster extends enchantUtil {
    ...
    public String[] getEnchantDescription() {
        return new String[]{
                MessageFormat.format("{0}Deal {1}+{2}%{0} damage vs. entities/n"
                        + "{0}above 50% HP", gray, red, damagePerLevel[0]),

                MessageFormat.format("{0}Deal {1}+{2}%{0} damage vs. entities/n"
                        + "{0}above 50% HP", gray, red, damagePerLevel[1]),

                MessageFormat.format("{0}Deal {1}+{2}%{0} damage vs. entities/n"
                        + "{0}above 50% HP", gray, red, damagePerLevel[2])
        };
    }
